...
let scriptId = null;
let scriptParamId = 85679; //id del script get_Colaborador.py
let areaParam = null;
...
window.onload = function(){
  ...
  var formNode = document.getElementById("appCont");
    ...
    if (key === 'param_id' ){
      ...
      scriptParamId = parseInt(qs[key]); //Utiliza get_Colaborador.py
    }
  ...
  if(us != "" && jw != ""){
    ...
    //---HIDE AND SHOW
    setSpinner();
    console.log('get Colaboradorooossssss')
    getColaborador();
    $('#divOptions').show();
    $('#title_report').show();
...
function getColaborador(){
  ...
  .then(res => {
    if (res.success) {
      //----SHOW STYLES
      if (res.response.Colaborador) {
        var Colaborador = res.response.Colaborador;
        $('#Colaborador').empty()
        $("#Colaborador").append('<option >--Seleccione--</option>')
        for (var i=0;i<Colaborador.length;i++) {
          // var Colaborador_value = Colaborador[i].replaceAll(' ','_').toLowerCase()
          var Colaborador_value = Colaborador[i]
          $("#Colaborador").append('<option value="'+Colaborador_value+'">'+Colaborador[i]+'</option>')
        }
      }
...
function runFirstElement(){
  ...
  firstElement =getFirstElement(Colaborador.value);
};
...
function getFirstElement(Colaborador){
  //---Hide style
  $("#divContent").hide();
  $('.load-wrapp').show();
  $('.title_tables').hide();
